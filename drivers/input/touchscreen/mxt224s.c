		...
		if (data->pdata->check_antitouch) {
			if (tch_area) {
				...
				if (data->num_fingers > 1) {
					...
					if (atch_area == 0) {
						pr_info("[TSP]Single touch Cal good condition : atch_area = 0)\n");
					} else {
						tmp = atch_area - 1;
						if ((tch_area > 10)
							|| ((tch_area <= 10)
		...
		if (object_type == TOUCH_MULTITOUCHSCREEN_T9) {
			id = msg[0] - data->finger_type;
				...
				if (id < 0 || id >= data->num_fingers) {
					Report_touch_number = 0;
					continue;
				} else {
					Report_touch_number = id + 1;
				}
	...
	for (read_page = 0 ; read_page < 6; read_page++) {
		for (node = 0; node < 64; node++) {
			...
			if ((mxt_refer_node[num] > MIN_VALUE)
				...
				state = 1;
				printk(KERN_ERR
				"[TSP] Mxt224 mxt_refer_node[%3d] = %5d\n",
